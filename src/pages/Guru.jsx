  ...
  return (
    ...
    >
      ...
      <div className="card relative overflow-hidden p-4 mb-4">
        <span className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-sage-500 to-brass-400" />
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-full bg-sage-500/10 text-sage-500 flex items-center justify-center shrink-0">
      ...
      <div className="card overflow-x-auto">
        <table className="table-shell">
          <thead>
            <tr className="border-b-2 border-sage-500/20">
              <th>Nama Lengkap</th>
              <th>NIP</th>
          ...
          <tbody>
            ...
            {filtered.map((g) => (
              <tr key={g.id} className="hover:bg-sage-500/[0.05] transition-colors">
                <td className="font-medium">
                  <button
                    type="button"
                    onClick={() => setProfilLihat(g)}
                    className="hover:underline hover:text-sage-500 text-left"
                  >
                    {g.nama_lengkap}
                  </button>
                </td>
                ...
                <td>{g.email}</td>
                <td>
                  <span className={`badge ${g.status === 'aktif' ? 'bg-sage-500/15 text-sage-500' : 'bg-red-900/10 text-red-900'}`}>
                    {g.status}
                  </span>
                </td>
                <td>
                  <div className="flex items-center gap-1 justify-end">
                    <button onClick={() => openEdit(g)} className="p-2 hover:bg-brass-400/10 rounded-lg text-brass-600"><Pencil size={15} /></button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink-950/50 backdrop-blur-sm p-4">
          <form onSubmit={handleSubmit} className="card relative overflow-hidden w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 rounded-full bg-sage-500/10 text-sage-500 flex items-center justify-center shrink-0">
      ...
      {profilLihat && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink-950/50 backdrop-blur-sm p-4">
          <div className="card w-full max-w-md p-0 relative overflow-hidden max-h-[90vh] overflow-y-auto">
            ...
            <div className="relative bg-gradient-to-br from-ink-900 to-ink-950 pt-8 pb-16 flex flex-col items-center">
